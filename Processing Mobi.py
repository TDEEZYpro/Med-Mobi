...
#     # then returns the doctors practice number when found for further use in booking
    ...
    while True:
        ...
        if c_input == 'n':
            ...
            mo = 'Mo: Please enter the Initials AND Surname of the doctor you want to book. e.g) "Chris Klopper" or "Frank Mahlangu"...'
            docInput = docInput = tags(mo)
            ...
            mo = 'Mo: Please enter the Office Email of the doctor you want to book. e.g) "xolanizulumedical@gmail.com" or "info@medicalhealth.co.za"...'
            docInput = docInput = tags(mo)
...
def get_dt_time():
    ...
    if bool(date) == True:
        ...
        input()
        #slots = from react
        final_slots = splitslots(slots)
        start_time_date = final_slots[0]
        end_time_date = final_slots[1]
        return [start_time_date, end_time_date]
...
def tags(mo):
        db = firestore.client()
        bot_respon = db.collection(u'Meessage').document('123457').update({'Message' : mo})
        user_input =""
        print(bot_respon)
        input()
